## ==============================================================================
## Arty Z7-20 Constraints for ECG-AI-Project (ecg_soc_board)
## Target Device: xc7z020clg400-1 (Digilent Arty Z7-20 Rev. B)
##
## File này kết hợp:
##  1. PHẦN ĐANG HOẠT ĐỘNG (Active Constraints) cho ecg_soc_board trong dự án.
##  2. TOÀN BỘ CÁC CHÂN NGOẠI VI ARTY Z7-20 REV. B (Master Constraints) ở dạng comment
##     sẵn sàng để mở (uncomment) khi bổ sung ngoại vi (RGB LED, HDMI, Pmod JB, SPI...).
## ==============================================================================

## -----------------------------------------------------------------------------
## PHẦN 1: CÁC RÀNG BUỘC ĐANG DÙNG CHO ECG-AI-PROJECT (ecg_soc_board)
## -----------------------------------------------------------------------------

## Clock Signal (125 MHz onboard oscillator)
set_property -dict { PACKAGE_PIN H16 IOSTANDARD LVCMOS33 } [get_ports { clk_i }]
create_clock -period 8.000 -name sys_clk_pin -waveform {0 4} [get_ports { clk_i }]

## Reset Button (BTN0 on board - active HIGH pressed)
## Wrapper inverts this to active-LOW reset for CV32E40X
set_property -dict { PACKAGE_PIN D19 IOSTANDARD LVCMOS33 } [get_ports { rst_ni }]
set_false_path -from [get_ports { rst_ni }]

## Fetch Enable Switch (SW0 on board: UP=1 run, DOWN=0 halt)
set_property -dict { PACKAGE_PIN M20 IOSTANDARD LVCMOS33 } [get_ports { fetch_enable_i }]
set_false_path -from [get_ports { fetch_enable_i }]

## 4 Green User LEDs
## LED0: core_sleep (1 when CPU finishes / WFI)
## LED1: cp_busy (coprocessor inference in progress)
## LED2: cp_done (coprocessor layer/model done)
## LED3: activity XOR parity indicator
...
## UART on Pmod JA (Top row)
## Pin 1 (JA1_P / Y18): UART_RX (FPGA input, connect to USB-UART TX)
## Pin 2 (JA1_N / Y19): UART_TX (FPGA output, connect to USB-UART RX)
## Pin 5: GND
## Pin 6: 3.3V
set_property -dict { PACKAGE_PIN Y18 IOSTANDARD LVCMOS33 } [get_ports { uart_rx_i }]
set_property -dict { PACKAGE_PIN Y19 IOSTANDARD LVCMOS33 } [get_ports { uart_tx_o }]

## Output and Input Delays referenced to MMCM generated clock (T = 24.0 ns)
set_output_delay -clock [get_clocks -of_objects [get_pins gen_mmcm.u_mmcm/CLKOUT0]] 6.000 [get_ports { led_o[*] }]
set_output_delay -clock [get_clocks -of_objects [get_pins gen_mmcm.u_mmcm/CLKOUT0]] 6.000 [get_ports { uart_tx_o }]
set_input_delay  -clock [get_clocks -of_objects [get_pins gen_mmcm.u_mmcm/CLKOUT0]] 6.000 [get_ports { uart_rx_i }]
...
## -----------------------------------------------------------------------------
## PHẦN 2: TOÀN BỘ CHÂN NGOẠI VI ĐẦY ĐỦ CỦA ARTY Z7-20 REV. B (MASTER CONSTRAINTS)
## (Bỏ dấu '#' ở đầu dòng và đổi tên port khi cần mở rộng thiết kế)
## -----------------------------------------------------------------------------

## Switches
#set_property -dict { PACKAGE_PIN M20    IOSTANDARD LVCMOS33 } [get_ports { sw[0] }]; #IO_L7N_T1_AD2N_35 Sch=SW0 (Đang dùng cho fetch_enable_i ở trên)
...
## Additional Buttons (BTN0 đang dùng cho rst_ni)
#set_property -dict { PACKAGE_PIN D19    IOSTANDARD LVCMOS33 } [get_ports { btn[0] }]; #IO_L4P_T0_35 Sch=BTN0 (Đang dùng cho rst_ni ở trên)
...
## Pmod Header JA (Pins 3, 4, 7, 8, 9, 10 còn lại)
## Note: Pin 1 (Y18) và Pin 2 (Y19) đang dùng cho UART ở Phần 1
